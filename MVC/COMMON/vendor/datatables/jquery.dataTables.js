...
(function (factory) {
  ...
  var DataTable = function (options) {
     ...
     *  @param {object} [oOpts] Optional parameters for modifying the rows to be used
  ...
  function _fnBrowserDetect(settings) {
    ...
    if (!DataTable.__browser) {
      ...
      // use the width of the scrollbar, while other browsers ensure the inner
   ...
   *  @param {boolean} [buseHidden=false] If true then use the hidden columns in the calc,
   *  @memberof DataTable#oApi
   */
  function _fnDrawHead(oSettings, aoSource, buseHidden) {
    ...
    for (i = 0, iLen = aoSource.length; i < iLen; i++) {
      ...
      for (j = iColumns - 1; j >= 0; j--) {
        if (!oSettings.aoColumns[j].bVisible && !buseHidden) {
  ...
  function _fnFilterCustom(settings) {
    ...
    for (var i = 0, ien = filters.length; i < ien; i++) {
      ...
      // Loop over each row and see if it should be used
  ...
  function _fnScrollDraw(settings) {
    ...
    if (scrollX === "") {
      ...
      // IE7 will make the width of the table when 100% use the scrollbar
  ...
  DataTable.models.oColumn = {
    ...
    /**
     * Flag to indicate if the column is searchable, and thus should be used
  ...
  DataTable.ext = _ext = {
     ...
     * and your logic decides if it should be used in the searching data set
     * or not.
     *
     ...
     * And the following return is expected:
     *
     * * {boolean} use the row in the searched result set (true) or not
     *   (false)
     *
